...
object DBQueries {
    ...
    def print_reports(): Unit = {
        ...
        res3.foreach{ case (country, runways) => println(s"Runway types in $country:"); runways.foreach(println)}

    }

    def report(): (Seq[(String, Int)], Seq[(String, Int)], Map[String, Seq[String]]) =
    {
        ...
        val map = res3.groupBy(_._1).mapValues(e => e.map(e => e._3).distinct)
        (res1, res2, map)
    }
    ...
    def query(name: String): Map[String, Map[String, Seq[(String, String)]]] =
    {
        ...
        res.groupBy(_._1).mapValues(_.map(e => (e._2, e._3, e._4))).mapValues(_.groupBy(_._1).mapValues(_.map(e => (e._2, e._3)))).toMap
    }

    def print_query(name: String): Unit = {
        val map = query(name)
